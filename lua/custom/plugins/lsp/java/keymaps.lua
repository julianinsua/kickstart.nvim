...
vim.api.nvim_buf_create_user_command(0, 'JdtCompile', function(ctx)
  require('jdtls').compile(ctx.fargs)
end, { bang = true, nargs = '?', complete = require('jdtls')._complete_compile })

vim.api.nvim_buf_create_user_command(0, 'JdtSetRuntime', function(ctx)
  require('jdtls').set_runtime(ctx.fargs)
end, { bang = true, nargs = '?', complete = require('jdtls')._complete_set_runtime })

vim.api.nvim_buf_create_user_command(0, 'JdtUpdateConfig', function()
  require('jdtls').update_project_config()
end, { bang = true })

vim.api.nvim_buf_create_user_command(0, 'JdtJol', function()
  require('jdtls').jol()
end, { bang = true })
...
vim.api.nvim_buf_create_user_command(0, 'JdtJshell', function()
  require('jdtls').jshell()
end, { bang = true })

-- NOTE: Above was originally taken from https://github.com/mfussenegger/nvim-jdtls/wiki/Sample-Configurations
